...
本机统一入口：桌面 → GitHub → flip-house-platform。目录与轮流开发规则见[仓库与协作约定](docs/仓库与协作约定.md)。
...
- 项目指引及 Jira 交付计划已整理：3 个 Epic（KAN-35/36/37）、15 张执行单（KAN-20–34）。业务代码仍待开发，不要将建单当成已上线能力。
...
核验基线：**PR #5 已于 2026-09-15 合入 main，合并提交 `5965b0a`**。笔记本 `7ffb139` 的登录/六段与原水电网站功能已统一，回归与构建证据见[版本核验](docs/当前实现与版本核验.md)。KAN-21 的项目授权、成员接口与真实验证不因「登录文件已存在」而关闭。
