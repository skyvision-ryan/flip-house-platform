...
- 本轮更新的是项目指引和开发计划，业务代码尚未按三张新 ticket 实现；不要将目标当成已上线能力。
...
| [本周三张开发 ticket](docs/本周开发Tickets_2026-09-15.md) | 代码范围、依赖、验收；尚未在 Jira 建单 |
...
核验基线：`0a459c1`（2026-09-15 拉取后的 origin/main）。笔记本整理记录的 `7ffb139` 未在当前仓库取得；其中所述登录和六阶段改动不能当成当前分支已有能力。

| 模块 | 当前代码包含 |
|---|---|
| 项目工作台 | 项目列表、按角色组合的小组件、预算/进展与更新记录 |
| 房产与新建项目 | 地址查找、模拟房产数据、多来源字段、冲突与来源追溯 |
| 项目总览与待办 | 五段清单、角色负责项、证据、D/J 节点确认、我的待办 |
| 文件和执行记录 | 文件/照片与步骤关联、水电账户、检查记录、采购记录 |
| 分析与预算 | 确定性交易计算、多版本分析、分析应用到预算、支出对照 |
| 助手 | 规则洞察与页面链接，尚无 LLM 调用 |
| 身份 | 演示角色通过 X-Actor 传入；有权限过滤规则，但还没有此分支可核验的独立账号登录 |
| 存储 | SQLite 与本地 uploads；空库启动会生成演示数据 |
