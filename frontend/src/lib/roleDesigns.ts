...
export type DesignPersona = 'jessie' | 'kody';
...
export interface RoleDesign {
  ...
  layout: 'house' | 'portfolio' | 'tasks' | 'register' | 'queue' | 'providers';
}
...
export function visibleDesignTasks(tasks: DesignTask[], houses: DesignHouse[], persona: DesignPersona, query: string, category = '全部', house = 'all') {
...
export function parseDesignPreferences(raw: string | null): Partial<Record<DesignPersona, DesignPreference>> {
  try {
    ...
    for (const persona of ['jessie', 'kody'] as const) {
