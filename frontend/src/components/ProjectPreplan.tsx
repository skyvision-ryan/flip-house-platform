...
export default function ProjectPreplan({ meta, plan, onChange, users, loading, creatorId }: Props) {
  ...
  const renderStage = (stage: PlanStage) => {
    ...
    return <Container key={stage.key} header={<Header variant="h2" counter={`(${ordinary.length})`} description={current ? '从这里开始；可先安排负责人和截止日期。' : '提前安排；创建后仍是未开始，关键节点按原规则推进。'}>{title}</Header>}>
  ...
  return <>
    ...
    {editing && <Modal visible header={`安排：${editing.title}`} onDismiss={() => setEditing(null)} footer={<Box float="right"><SpaceBetween direction="horizontal" size="xs"><Button variant="link" onClick={() => setEditing(null)}>取消</Button><Button variant="primary" onClick={() => {
      ...
      <SpaceBetween size="l">
        ...
        <Box color="text-body-secondary">这里只保存草稿。创建后任务为“未开始”，审核人沿用现有规则，默认为创建者。</Box>
      </SpaceBetween>
    </Modal>}
  </>;
}
...
export function PlanSummary({ meta, plan, users, review = false }: { meta: Meta; plan: TaskPlan; users: UserBrief[]; review?: boolean }) {
  ...
  return <SpaceBetween size="l">
    <Container header={<Header variant="h2">安排摘要</Header>}>
      <SpaceBetween size="m">
        ...
        <Box color="text-body-secondary">未分派任务也会创建，可在项目总览继续安排。</Box>
      </SpaceBetween>
    </Container>
    <Container header={<Header variant="h2">邮件提醒</Header>}>
      <SpaceBetween size="m">
    ...
    {preview && <Modal visible size="large" header="安排说明预览 · 尚未发送" onDismiss={() => setPreview(false)} footer={<Box float="right"><Button onClick={() => setPreview(false)}>关闭</Button></Box>}>
      <SpaceBetween size="l"><Alert type="info">这是按负责账号合并的内容预览。邮件接通后再提供实际收件地址、发送与回执。</Alert>{recipients.map((r, index) => <Container key={r.user?.id ?? index} header={<Header variant="h3"><PersonAvatar user={r.user} /></Header>}><SpaceBetween size="s">{r.items.map((i) => <div key={`${i.stage}-${i.title}`}><Box fontWeight="bold">{i.title}</Box><Box color="text-body-secondary">{stageKeyLabel(meta.stage_groups, i.stage_key, i.stage)} · {i.future ? '提前准备' : '当前阶段'} · 截止 {i.due || '未设定'}</Box></div>)}</SpaceBetween></Container>)}</SpaceBetween>
    </Modal>}
    {review && <Box variant="small" color="text-body-secondary">创建前可返回修改，全部安排会保留。</Box>}
  </SpaceBetween>;
}

export function PlanReview({ meta, plan }: { meta: Meta; plan: TaskPlan }) {
  return <Table variant="container" header={<Header variant="h2" description="全部模板任务都会创建；已分派和待分派都保留。">全流程安排</Header>} items={meta.stage_groups ?? []} trackBy="key" columnDefinitions={[
