 ...
 */
export default function TaskAssignModal({ projectId, task, onDone, onConflict, onDismiss }: {
  projectId: number; task: Task; onDone: (t: Task) => void; onConflict: () => void; onDismiss: () => void;
}) {
  ...
  const options: SelectProps.Options = [
    ...(task.assignee ? [{ value: UNASSIGN, label: '取消分派', description: '任务回到待分派' }] : []),
  ...
  const isReassign = changingPerson && !!task.assignee;
  const needJoin = !!target && !target.member;
  const dueChanged = (due || '') !== (task.due_at ?? '');
  const nothing = !changingPerson && !dueChanged;
  ...
  return (
    <Modal
      visible
      onDismiss={onDismiss}
      header={task.assignee ? `调整安排：${task.title}` : `分派：${task.title}`}
      footer={
        <Box float="right">
