...
import DirectorDesign from './DirectorDesign';
import FounderDesign from './FounderDesign';
import type { LeadershipPersona, LeadershipPreview } from '../../lib/leadershipDesign';
import { parseDesignPreferences } from '../../lib/roleDesigns';

export default function LeadershipDesignComparison({ persona, preview, storageKey }: {
  persona: LeadershipPersona; preview: LeadershipPreview; storageKey: string;
}) {
  ...
  const Workspace = persona === 'david' ? DirectorDesign : FounderDesign;
  const save = () => {
    try {
  ...
  return <ContentLayout maxContentWidth={1680} header={<Header variant="h1" description="比较职责所需的信息组织方式，选择更顺手的一种。">{preview.role_title} · 设计比较</Header>}>
    ...
    <div className="ui-rd-preview-caption"><strong>{current.title} · 可点击工作界面</strong><span>三个方案共用同一批合成记录；数字可追溯，问题可展开；模型、业务日期与当前任务状态分别呈现。</span></div>
