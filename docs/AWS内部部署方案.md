...
更新：2026-09-17。状态：**目标架构已确定，基础设施与代码待实施**。架构本身未因 09-17 会议改变。

最新约束：小企业首次内部试用、iPhone 为主、后续业务逐步采用标准 AWS 服务。日期：09-18 技术 demo（**原计划**）、09-21 至 09-25 AWS 内部上线（**原计划**）。

> **09-17 会议的影响（待重新确认）**：会议把重心转向「团队从一个新项目开始每天使用」，并暂定 09-25 周五全员试用。该试用与本方案的 09-21–09-25 上线窗口**共用同一周**，两者的先后与范围需要重新确认——是先完成 AWS 迁移再让团队用，还是先在现有演示环境让团队用起来。本页不替负责人选定。
>
> 会议同意第一版用**账号密码登录**，这是使用方式，**不取消 Cognito**，也不降低正式环境的安全要求（见第 3 节）。
...
- **首批名单与职责待 Jesse 确认**（09-17）：最终人员、代号映射、外部参与者是否登录尚未定。会议文字稿中的姓名有转写错误，不据此建立任何账号。
- **首次使用的数据输入**：从一个真实的新项目开始，正式环境不灌示例数据（`SEED_DEMO=0`）。这是环境要求，不涉及删除现有数据。
