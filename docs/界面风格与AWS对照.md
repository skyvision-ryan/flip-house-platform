...
| A09 | `lib/role.ts:6` + `components/OwnerTag.tsx:12,21` | 角色 tier 四色 `#7A3EE8`/`#0972D3`/`#0E8A8A`/`#7D8998`，圆标底色 | 同 A07 | 无对应令牌 | 颜色编码的是**角色层级**，属分类信息不是状态 | 违反 | `OwnerDot` 改中性底，级别用文字；`colorOf` 随之成死代码 | KAN-49 |
...
* Open Sans 由 `@fontsource/open-sans` **实际加载**（400/600/700，自托管不走外链）。此前只在字体栈里写了名字，macOS 不自带这个字体，`BASE_STACK` 实际落到第二顺位 Helvetica Neue——等于没生效。**这一条引入了新依赖**，与本票「不加新依赖」的反例冲突，按用户当轮明确指令执行。
