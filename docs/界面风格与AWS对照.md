...
- 保留 Cloudscape 控件、字号、行高、字重和状态色；Ryan 09-24 已选择 B「轻量协作工作区」，协作区域改用连续平面与细分隔线，减少重复容器边框。字体按 Ryan 最新指定，全站正文 / 标题 / 控件统一为 `"Helvetica Neue", Helvetica, Arial, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "Noto Sans SC", "Source Han Sans SC", sans-serif`，不再加载 Open Sans。保留已授权的橙底深字主操作，链接 / 选中为官方蓝色；橙色是产品品牌例外，不是声称 Cloudscape 默认主按钮为橙色。
- B 已应用到项目头部、任务安排 / 右侧摘要、工作台项目关注 / 交接、我的事项 / 处理面板；顶栏改为浅色，辅助区域用官方中性浅灰令牌，橙色集中于主要操作。手机宽度下，选择任务进入摘要并可返回列表；员工色、状态、证据与 D/J 控制保留。
- `/design-choices` 提供 B / Linear、Jira、shadcn、Carbon、Fluent、Mantine 六种参考方向的适配草图，可切换、筛选、点击任务查看详情 / 活动；使用合成资料，不写业务数据，也未替换组件库。偏好仅保存在本机；演示模式侧栏提供入口。
...
- B 本地验证：`check_local.py` 后端 86、前端 110、工具 17 + 24、构建与 diff 全通过。浏览器验证实际项目任务 / 摘要与账号待办在桌面及 390×844 下的选择、返回；参考页六方向切换、搜索空态与活动联动。助理实际登录可见全局项目关注，无财务页签；手机模拟不代表真机 iPhone 验收。
- 验收按真实页面进行：清爽、只说明、只编号、双开四种组合；切页 / 刷新持久化；编号复制后可粘贴；说明关闭不隐藏任务状态、来源或风险；桌面与窄屏无页面级横向溢出。浏览器窄屏模拟与真机 iPhone 验收分开记录。

## 历史审计 · KAN-49 / KAN-64

以下保留当时证据。其中字体、palette fallback 与 hexGuard 白名单已被上方 2026-09-24 规则替代，不作为当前开发路径。
